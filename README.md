...
- [`corpus/`](corpus/README.md) — five-broadcast timeline evaluation corpus with hand-marked court/scene evidence, captain-verified rally-active intervals, and public URLs + media-time provenance only; its schema and offline validation support Phase-1 camera-grammar work.
...
composition to the MoveNet TF.js adapter (its graph checkpoint loads from
TensorFlow Hub and is not bundled). BlazePose is work in progress: its menu
...
For the overlay Shadow DOM/font packaging diagnosis, manual-labeling reliability
reproduction, and the repeatable live-page smoke sequence using the captain's
already-open Chrome, see [`docs/overlay-ui.md`](docs/overlay-ui.md),
[`docs/manual-labeling.md`](docs/manual-labeling.md), and [`docs/e2e-smoke.md`](docs/e2e-smoke.md).
