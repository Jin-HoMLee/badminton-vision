...
(function installBlazePoseAdapter(root, factory) {
  ...
  class BlazePoseAnalyzer {
    ...
    _decodeOutput(outputTensor, inputDims) {
      ...
      if (!data || typeof data.length !== 'number') throw new Error('BlazePose output tensor has no data');
      let shape = Array.from(outputTensor?.dims || []);
      while (shape.length > 0 && shape[0] === 1) shape = shape.slice(1);

      let numPoses;
      let valuesPerKeypoint;
      if (shape.length >= 2 && shape[shape.length - 2] === 17 && [3, 4].includes(shape[shape.length - 1])) {
        valuesPerKeypoint = shape[shape.length - 1];
        numPoses = shape.length === 2 ? 1 : shape[shape.length - 3];
      } else {
        // The injected/test seam may provide a flat tensor without dimensions.
        // Prefer the four-value BlazePose export (x,y,z,score) when length
        // proves it; retain support for the older x,y,score layout.
        valuesPerKeypoint = data.length % (17 * 3) === 0 ? 3 : 4;
        numPoses = Math.floor(data.length / (17 * valuesPerKeypoint));
      }
      if (!Number.isInteger(numPoses) || numPoses < 1) return [];
      numPoses = Math.min(4, numPoses);
      const coordinateDimension = Number(inputDims?.[2]) || 256;
      const normalizeCoordinate = (value) => {
        const number = Number(value);
        if (!Number.isFinite(number)) return 0;
        return Math.max(0, Math.min(1, Math.abs(number) > 1.5 ? number / coordinateDimension : number));
      };
      const valueAt = (pose, keypoint, component) => {
        const flatIndex = (pose * 17 + keypoint) * valuesPerKeypoint + component;
        return Number(data[flatIndex]);
      };
      const poses = [];
      ...
      for (let poseIdx = 0; poseIdx < numPoses; poseIdx++) {
        ...
        for (let kpIdx = 0; kpIdx < 17; kpIdx++) {
          const x = normalizeCoordinate(valueAt(poseIdx, kpIdx, 0));
          const y = normalizeCoordinate(valueAt(poseIdx, kpIdx, 1));
          const confidence = Math.max(0, Math.min(1, Number(valueAt(poseIdx, kpIdx, valuesPerKeypoint === 4 ? 3 : 2)) || 0));
          if (confidence > 0.1) {
            keypoints.push({ name: KEYPOINT_NAMES[kpIdx], x, y, confidence });
            validKeypoints += 1;
          } else {
            keypoints.push({ name: KEYPOINT_NAMES[kpIdx], x, y, confidence: null });
          }
        }
        if (validKeypoints >= 4) {
    ...
    async analyze(sample) {
      try {
        ...
        // Decode output
        const poses = this._decodeOutput(poseOutput, [1, 3, 256, 256]);
    ...
    async _readFramePixels(frame) {
      ...
      if (frame.data && frame.width && frame.height) {
        ...
        if (Number.isInteger(channels) && channels >= 3) {
          // Convert RGBA (or another packed pixel format) to RGB
