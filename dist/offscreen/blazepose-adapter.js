...
(function installBlazePoseTfjsAdapter(root, factory) {
  ...
  const MODEL = Object.freeze({
    ...
    // Graph output node names from the model's own signature.outputs. The
    // model declares five named outputs; requesting these two explicitly by
    // name (rather than positionally indexing a bare model.execute(input)
    // result) is required for correct results: the graph's own declared
    // output order is [output_poseflag, activation_heatmap, world_3d,
    // ld_3d, activation_segmentation], not [landmarks, presence].
    landmarkOutputName: 'ld_3d',
    presenceOutputName: 'output_poseflag'
  });
  ...
  class BlazePoseAnalyzer {
    ...
    async infer(frame, context = {}) {
      ...
      let output = null;
      try {
        // Request the landmark and presence outputs by name: the graph's own
        // default output order does not put landmarks first (see MODEL's
        // landmarkOutputName/presenceOutputName comment above).
        output = this.model.execute(input, [MODEL.landmarkOutputName, MODEL.presenceOutputName]);
